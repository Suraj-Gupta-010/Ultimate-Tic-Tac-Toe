...
internal_functioning: bool = True
PRUNING_PRIORITY = "nothing"
LAST_BEST_MOVE = None
...
def order_moves(
    ...
    if pruning_priority == "nothing":
        ...
        return sorted(legal_moves, key=priority)

    elif pruning_priority == "heuristic":
        scored = []
        ...
        for board, cell in legal_moves:
            ...
            # corners
            elif cell in [1, 3, 7, 9]:
                score += 50

            # immediate local-board win
            ...
            scored.append((score, (board, cell)))

        scored.sort(reverse=True)

        return [move for score, move in scored]

    elif pruning_priority == "minimax":

        if LAST_BEST_MOVE in legal_moves:
            return (
                [LAST_BEST_MOVE]
                + [m for m in legal_moves if m != LAST_BEST_MOVE]
            )
...
def minimax(nought_: dict[str, list[int]], cross_: dict[str, list[int]],
    ...
    legal_moves = order_moves(legal_moves, nought_, cross_, player, PRUNING_PRIORITY)
    ...
    if maximizing:
        best = float('-inf')
        for board, cell in legal_moves:
            if internal_functioning:
                print(f"{indent}MAX trying ({board}, {cell})")
            cross_[board].append(cell)
            ...
            if internal_functioning:
                print(f"{indent}MAX ({board}, {cell}) scored {score}, best so far = {max(best, score)}")

            best = max(best, score)
            alpha = max(alpha, best)
            if beta <= alpha:
                if internal_functioning:
                    print(f"{indent}PRUNED (alpha={alpha}, beta={beta})")
                break
        return best
        ...
        for board, cell in legal_moves:
            if internal_functioning:
                print(f"{indent}MIN trying ({board}, {cell})")
            nought_[board].append(cell)
            next_board = chr(64 + cell)
            if has_won(nought_[next_board]) or has_won(cross_[next_board]) or \
               len(nought_[next_board]) + len(cross_[next_board]) == 9:
            ...
            if internal_functioning:
                print(f"{indent}MIN ({board}, {cell}) scored {score}, best so far = {min(best, score)}")
    ...
    if PRUNING_PRIORITY == "minimax":
        ...
        for current_depth in range(1, depth + 1):
            ...
            for board, cell in legal_moves:
                ...
                if (
                    has_won(nought_[next_board])
                    or has_won(cross_[next_board])
                    or len(nought_[next_board])
                    + len(cross_[next_board]) == 9
                ):
                    next_board = "0"
