...
// DEBUG
function pythonGameplaySetup() {
    console.log("Click once to confirm initiation");
    document.addEventListener("click", () => {
        console.log("Python Gameplay Setup Initiated");
        navigator.clipboard.writeText("Initiation Complete");
        console.log("Please close this window for expected results, as per the calibration.");
    }, { once: true });
    debug = true;
}
...
document.querySelectorAll(".cell").forEach(cell => {
    ...
    cell.addEventListener("click", () => {
        ...
        console.log(evaluate(cross_turns, nought_turns, "✖"))

        if (!game_over){
            renderOpenBoards("0", cell); // converts old clicked-cell, skips current
            let next_board = String.fromCharCode(64 + +cell.dataset.index)
